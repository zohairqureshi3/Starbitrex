...
const Index = () => {
  ...
  useEffect(() => {
    if (orders && withdraws && deposits && adminDeposits && adminWithdraws) {
      const combineData = [...orders, ...withdraws, ...deposits, ...adminDeposits, ...adminWithdraws];
      setTransactions(combineData)
    }
  }, [orders, withdraws, deposits, adminDeposits, adminWithdraws])
  ...
  useEffect(() => {
    if (currencyDataState && currencyDataState.length) {
      ...
      setSelectedCurrency(currencyDataFilter?.find(row => row.symbol == coin));
  ...
  const changeCurrency = (currency) => {
    ...
    window.history.pushState("", "", '/activity/' + currency.symbol);
  }
                ...
                <Accordion.Header>
                  <div className="transcations-accord-content">
                    ...
                    <div className="text-white dd-icon d-flex align-items-center">
                      <h5 className="m-0" style={{ color: selectedCurrency?.color }}>{row.sign} {parseFloat(row.amount).toFixed(4)} {row.symbol}</h5>
                    </div>
                  </div>
                </Accordion.Header>
                    ...
                    <div className="accordion-content">
                      ...
                      <div className="accordion-content-activity">
                        ...
                        <div className="activity">
                          ...
                          <p className="text-light mb-0">{row.sign} {parseFloat(row.amount).toFixed(4)} {row.symbol}</p>
                        </div>
                        <div className="activity">
