...
const Index = () => {
  ...
  const [fiatAmounts, setFiatAmounts] = useState([]);
  const [showConfirmation, setShowConfirmation] = useState(false);
  const handleCloseConfirmation = () => setShowConfirmation(false);
  const handleShowConfirmation = () => setShowConfirmation(true);

  const dispatch = useDispatch();
  const fiatCurrencies = useSelector((state) => state?.fiatCurrency?.fiatCurrencies?.allFiatCurrencies);
  const currencies = useSelector((state) => state?.currency?.currencies?.allCurrencies);
  const userId = useSelector((state) => state?.user?.user?._id);
  const amountsData = useSelector((state) => state?.accounts?.account?.amounts);
  const fiatAmountsData = useSelector((state) => state?.accounts?.account?.fiatAmounts);
  ...
  useEffect(() => {
    if (fiatCurrencies?.length > 0) {
      setFiatCurrencyData(fiatCurrencies);
    }
  }, [fiatCurrencies]);
  ...
  useEffect(() => {
    if (fiatAmountsData) {
      setFiatAmounts(fiatAmountsData);
    }
  }, [fiatAmountsData]);
  ...
  const userCoinChange = (coin) => {
    if (coin?._id) {
      ...
      setUserCoin(coin);
      if (coin?.isFiat) {
        setHaveCoins(fiatAmounts?.find((row) => row?.fiatCurrencyId == coin?._id)?.amount);
      }
      else {
      ...
      axios
        .get(
          `https://min-api.cryptocompare.com/data/price?fsym=${coin?.symbol}&tsyms=ETH,LINK,AVAX,DOGE,BCH,LTC,TRX,BNB,ADA,BTC,USDT&api_key=6f8e04fc1a0c524747940ce7332edd14bfbacac3ef0d10c5c9dcbe34c8ef9913`
        )
        .then((res) => {
  ...
  useEffect(async () => {
    if (fiatCurrencyData) {
      let found = await fiatCurrencyData?.find(currency => currency?.symbol == symbol);
      userCoinChange(found);
    }

    if (currencyData) {
      let exchangeCurr = await currencyData?.find(currency => currency?.symbol == 'USDT');
      setExchangeCoin(exchangeCurr);
    }
  }, [currencyData, fiatCurrencyData, amounts])
  ...
  const exchange = () => {
    ...
    if (exchangeCoin?.isFiat) {
      setCurrencyData(currencies);
      setFiatCurrencyData(fiatCurrencies);
    }
    else {
      setCurrencyData(fiatCurrencies);
      setFiatCurrencyData(currencies);
    }
              ...
              <div className="container">
                <div className="crypto-fields-dd">
                  <div className="crypto-have">
                    <div className="dropdown crypto-have-dropdown">
                      ...
                      <ul className="dropdown-menu dropdown-scrollable" aria-labelledby="dropdownMenuLink">
                        {fiatCurrencyData && fiatCurrencyData.length > 0 && fiatCurrencyData.filter((c) => c.symbol !== 'USD' || c.symbol !== 'USDT').map((currency) => (
                          <li>
                  ...
                  <div className="crypto-have" style={{ marginTop: "-10px" }}>
                    ...
                    <h3 className="text-white m-0">
                      <input type="number" value={amountErr ? "" : (exchangeAmount == 0 ? exchangeAmount : parseFloat(exchangeAmount).toFixed(5))} disabled />
                    </h3>
                  </div>
                </div>
