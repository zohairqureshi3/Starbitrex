...
import { showAllUsers, deleteUser, editUser, referralsPerId, userDirectLogin, deleteUsers, editUsers, showSalesAgents, showAgents, importCSVFile, getCountries, exportLeadsToFile, getUser, getUserDetails } from '../../redux/users/userActions';
...
import FilterStatusComponent from '../../components/FilterStatusComponent';
...
const Leads = () => {
   ...
   const salesAgents = useSelector(state => state.users?.salesAgents);
      ...
      if(roleData?.length > 0)
      {
         ...
         else
         {
            if (salesAgents?.length > 0) {
               let allAgents = salesAgents?.map(agen => { return { label: agen.firstName + agen.lastName, value: agen._id } });
               setAgentUsers(allAgents);
               setAssignToShow(salesAgents);
               setAssignedTo({ label: allAgents?.[0]?.label, value: allAgents?.[0]?.value });
            }
         }
      }
      
   }, [salesAgents, agentUsersData]);
   ...
   useEffect(async () => {
      if(roleData?.length > 0)
      {
         ...
         {
            await dispatch(showSalesAgents());
         }
      }
   }, [roleData])
   ...
   const moveUsersInBulk = () => {
      if (selectedUsersIds?.length > 0) {
         Swal.fire({
            title: `Are you sure you want to move user(s) to client category?`,
            html: '',
            showCloseButton: true,
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
   ...
   const columns = [
            ...
            <div>
               <div className='search1'>
                  <div className="dropdown search-custom-dropdown dropdown-check-nostyle">
                     <button onClick={handleCountryDropdownShow} className="dropbtn" id="country-dropdown-show">Search</button>
      ...
      },
      {
         name: 'Affiliator',
         selector: row => {
            if (row?.users?.affiliator)
               return `${row?.users?.affiliator?.[0]?.firstName} ${row?.users?.affiliator?.[0]?.lastName}`
            else
               return '-'
         },
         cell: (row) => {
            let affiliator = row?.users?.affiliator?.[0] ? row?.users?.affiliator?.[0] : '';
            return (
               <>
                  {affiliator ?
                     <>
                        <span data-tip={`${affiliator?.firstName} ${affiliator?.lastName}`}>{affiliator?.firstName} {affiliator?.lastName}</span>
                        <ReactTooltip />
                     </> : '-'}
               </>
            );
      ...
      },
      {
         ...
         selector: row => {
            return (
               <>
                  <span data-tip={(timePassed(row?.users?.lastLoginAt) && row?.users?.isLogin === true) ? timePassed(row?.users?.lastLoginAt) : 'Offline'}>{(timePassed(row?.users?.lastLoginAt) && row?.users?.isLogin === true) ? timePassed(row?.users?.lastLoginAt) : 'Offline'}</span>
                  <ReactTooltip />
               </>
            )
         },
      ...
      return (
         <div className='d-flex w-100 justify-content-between mt-5 mb-4'>
            <div className="btn-group btn-group-toggle">
               <button className="btn btn-secondary btn-sm me-1 p-1" style={{ position: 'relative'}} onClick={() => moveUsersInBulk()}>Move to Clients</button>
      ...
      if (file) {
         ...
         reader.onloadend = () => {
            ...
            if (
               ...
               Swal.fire({
                  text:
                     "The file must be a file of type: csv,xlsx",
                  icon: "error",
   ...
   useEffect(async () => {
      ...
      if (csvFile) {
         ...
         await dispatch(importCSVFile(formData, type, id, uid, clientType));
      }
      setImgLoader(false);
   ...
   const handleExportLead = async () => {
      ...
      const uid = data?._id;
      const clientType = 1;
      await dispatch(exportLeadsToFile(type, id, uid, clientType));
   }
                  ...
                  <div className="content-box">
                     ...
                     <div className='d-flex'>
                        {
                           show && permissionName && permissionName.length > 0 && permissionName.includes('add_user') ?
                              <Link to='/admin/add-user'><button className="btn btn-default">Add Lead</button></Link>
                              :
                              null
                        }
                        {
                           show && permissionName && permissionName.length > 0 && permissionName.includes('add_user_with_csv_file') ?
                              <div className="import-csv-btn">
                                 <label htmlFor="file" className="btn btn-default">Import Leads</label>
                                 <input style={{ display: "none" }} id="file" type='file' name="csvFile" accept=".csv,.xlsx" onChange={handleCSVChange} />
                              </div>
                              :
                              null
                        }
                        {
                           show && permissionName && permissionName.length > 0 && permissionName.includes('export_leads_to_file') ?
                              <div className="import-csv-btn">
                                 <button className="btn btn-default" onClick={handleExportLead}>Export Leads</button>
                              </div>
                              :
                              null
                        }
                     ...
                     <div>
                        ...
                        <div className="form-group col-md-12 mb-3">
                           ...
                           <FilterComponent aria-label="Default search example" style={{ margin: 'auto' }} onFilter={e => setFilterText(e.target.value)} onClear={handleClear} filterText={filterText} />
                        </div>
                     </div>
