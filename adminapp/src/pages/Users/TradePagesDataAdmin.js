...
import { Tabs, Tab, Table, Modal, InputGroup, FormControl } from "react-bootstrap";
import { Link } from "react-router-dom";
import DataTable, { createTheme } from "react-data-table-component";
import { stopLeverageOrder, updateLeverageOrder, startLeverageOrder } from '../../redux/leverageOrder/leverageOrderActions';
...
function TradeOrdersDatatables({ setLoader, selectedRow, avbl, updateAvbl, amounts, token, currencyData, userOrders, setSelectedRow, setTP, setSL, TP, SL, rates, setTrailingPrice, trailingPrice }) {
      ...
      useEffect(() => {
            userOrders && userOrders?.length && userOrders.filter((row) => row.status == 1).map(async (row) => {
                  ...
                  <Modal className="withdrawal-modal limit-modal" show={showLimit} centered onHide={handleCloseLimit} backdrop="static">
                        ...
                        <Modal.Body>
                              <div className="buy-tabs">
                                    ...
                                    <span className="text-white">Closing Price USDT</span>
                                    ...
                                    <span className="text-white">Closed Qty ETH</span>
                                    <InputGroup className="mb-3">
                                          <FormControl
                                                type="number"
                                                step="0.1"
