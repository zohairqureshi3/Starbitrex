...
import { deleteFiatCurrency, showAllFiatCurrencies } from '../../redux/fiatCurrency/fiatCurrencyActions';
                  ...
                  <div className="content-box">
                     <h3>Fiat Currencies Details</h3>
                     {
                        permissionName && permissionName.length > 0 && permissionName.includes('add_fiat_currency') ?
                           <Link to='/admin/add-fiat-currency'><button className="btn btn-default">Add Fiat Currency</button></Link>
                           :
                           null
                     }
                        ...
                        <table className="table">
                           <thead className="table_head">
                              <tr>
                                 ...
                                 {/* <th>Currency Icon</th> */}
                                 {
                                    permissionName && permissionName.length > 0 && permissionName.includes('edit_fiat_currency', 'delete_fiat_currency') ?
                                       <th>Action(s)</th>
                                       :
                                       null
                                 }
                              ...
                              {currencies && currencies?.length > 0 && currencies?.filter(row => row?.isFiat === true && row?.status == true)?.map((currency) => {
                                 return (
                                    <tr key={currency._id}>
                                       ...
                                       <td>
                                          {
                                             permissionName && permissionName.length > 0 && permissionName.includes('edit_fiat_currency') ?
                                                <Link to={`/admin/edit-fiat-currency/${currency._id}`} className='btn btn-primary me-2 text-decoration-none text-light'>Edit</Link>
                                                :
                                                null
                                          }
                                          {
                                             permissionName && permissionName.length > 0 && permissionName.includes('delete_fiat_currency') ?
